...
@MainActor
actor FrameSyncCoordinator: Sendable {
    ...
    func addFrame(_ frame: DualCameraFrame) async {
        ...
        case .front:
            await addFrameToQueue(frame, queue: &frontFrameQueue)
        case .back:
            await addFrameToQueue(frame, queue: &backFrameQueue)
        default:
            break
        }
    ...
    private func addFrameToQueue(_ frame: DualCameraFrame, queue: inout [DualCameraFrame]) async {
