...
@available(iOS 15.0, *)
final class ModernCameraSessionConfigurator: @unchecked Sendable {
    ...
    private func configureVideoFormat(for device: AVCaptureDevice, quality: VideoQuality) async {
        await Task { @Sendable [weak self] in
            let targetDimensions = quality.dimensions
            
            guard let format = device.formats.first(where: { format in
                let dimensions = CMVideoFormatDescriptionGetDimensions(format.formatDescription)
                return Int32(dimensions.width) >= Int32(targetDimensions.width) &&
                       Int32(dimensions.height) >= Int32(targetDimensions.height) &&
