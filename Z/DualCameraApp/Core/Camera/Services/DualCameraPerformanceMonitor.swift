...
@MainActor
actor DualCameraPerformanceMonitor: Sendable {
    ...
    private func updateMemoryUsage(currentTime: Date) async {
        ...
        if timeInterval >= 5.0 { // Check every 5 seconds
            ...
            currentMetrics.memoryUsage = memorySnapshot.usageRatio
            currentMetrics.memoryPressure = memorySnapshot.pressure
    ...
    private func updateThermalState(currentTime: Date) async {
        ...
        if timeInterval >= 2.0 { // Check every 2 seconds
            ...
            let snapshot = ThermalSnapshot(
                ...
                state: thermalState,
                pressure: thermalPressure.pressure,
                trend: thermalPressure.trend
            )
            ...
            currentMetrics.thermalPressure = thermalPressure.pressure
    ...
    private func checkPerformanceThresholds() async {
        ...
        if currentMetrics.thermalState.rawValue >= ThermalState.serious.rawValue {
            if currentMetrics.thermalState.rawValue >= ThermalState.critical.rawValue {
                criticalIssues.append("Thermal state critical: \(currentMetrics.thermalState.description)")
            } else {
                warnings.append("Thermal state elevated: \(currentMetrics.thermalState.description)")
            }
        }
...
struct DualCameraPerformanceMetrics: Sendable {
    ...
    var thermalPressure: ThermalPressure = .none
...
struct PerformanceRecommendation: Sendable {
    ...
    enum RecommendationPriority: String, Sendable {
        case low = "Low"
        case medium = "Medium"
        case high = "High"
        case critical = "Critical"
