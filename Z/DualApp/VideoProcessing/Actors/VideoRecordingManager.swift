...
actor VideoRecordingManager: Sendable {
    ...
    func startRecording() async throws -> RecordingSession {
        ...
        var session = RecordingSession(configuration: configuration)
    ...
    func stopRecording() async throws -> RecordingSession {
        guard isRecording, var session = activeSession else {
    ...
    func pauseRecording() async throws {
        ...
        isPaused = true
        pausedDuration = Date().timeIntervalSince1970
        
        // Pause asset writers
        for (_, writer) in assetWriters {
            // Note: AVAssetWriter doesn't support pause/resume
        }
    ...
    func resumeRecording() async throws {
        ...
        isPaused = false
        totalPausedDuration += Date().timeIntervalSince1970 - pausedDuration
    ...
    private func createAudioInput(for output: RecordingOutput, writer: AVAssetWriter) async throws {
        let audioSettings: [String: Any] = [
            ...
            AVNumberOfChannelsKey: configuration.stereoRecordingEnabled ? 2 : 1,
    ...
    private func calculateRecordingDuration() -> TimeInterval {
        ...
        if isPaused {
            duration -= Date().timeIntervalSince1970 - pausedDuration
        }
