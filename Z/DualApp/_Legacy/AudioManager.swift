...
class AudioManager: NSObject {
    ...
    private var audioLevelBuffer: [Float] = Array(repeating: 0.0, count: 100)
    private var audioLevelBufferCount: Int = 0
    ...
    override init() {
        super.init()
        loadSavedSettings()
        // Audio session and engine setup deferred until recording starts
    }
    ...
    private func activateAudioSessionForRecording() {
    ...
    private func initializeAudioEngineIfNeeded() {
        ...
        print("Audio engine initialized (not started yet)")
    }
    
    func prepareForRecording() throws {
        activateAudioSessionForRecording()
    ...
    func stopRecording() {
        ...
        print("Audio recording stopped and session deactivated")
    }
    ...
    private func processAudioBuffer(_ buffer: AVAudioPCMBuffer) {
        ...
        if audioLevelBufferCount < 100 {
            audioLevelBuffer[audioLevelBufferCount] = processedValue
            audioLevelBufferCount += 1
        } else {
            for i in 0..<99 {
                audioLevelBuffer[i] = audioLevelBuffer[i + 1]
            }
            audioLevelBuffer[99] = processedValue
        }
    }
    ...
    func getAudioLevelBuffer() -> [Float] {
        return Array(audioLevelBuffer[0..<audioLevelBufferCount])
    }
    ...
    func getAverageAudioLevel() -> Float {
        guard audioLevelBufferCount > 0 else { return 0.0 }
        
        let sum = audioLevelBuffer[0..<audioLevelBufferCount].reduce(0, +)
        return sum / Float(audioLevelBufferCount)
    }
    
    func getPeakAudioLevel() -> Float {
        guard audioLevelBufferCount > 0 else { return 0.0 }
        
        return audioLevelBuffer[0..<audioLevelBufferCount].max() ?? 0.0
    }
    ...
    // Deprecated: Use prepareForRecording() instead
