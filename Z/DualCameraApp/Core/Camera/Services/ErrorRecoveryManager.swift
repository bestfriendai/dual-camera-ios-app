...
@MainActor
actor ErrorRecoveryManager: Sendable {
    ...
    func removeRecoveryStrategy(_ strategy: ErrorRecoveryStrategy) async {
        recoveryStrategies.removeAll { $0.id == strategy.id }
    ...
    private func attemptRecovery(for errorRecord: ErrorRecord) async {
        ...
        if errorRecord.recoveryAttempts >= maxRetryAttempts {
            eventContinuation.yield(.recoveryFailed(errorRecord, "Maximum retry attempts exceeded"))
            return
        }
