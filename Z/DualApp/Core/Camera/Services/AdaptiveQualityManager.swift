...
actor AdaptiveQualityManager: Sendable {
    ...
    private var adaptiveStrategies: [any AdaptiveStrategy] = []
    ...
    init() {
        ...
        // Initialize quality levels and strategies asynchronously
        Task {
            await setupQualityLevels()
            await setupAdaptiveStrategies()
        }
    }
    ...
    func addAdaptiveStrategy(_ strategy: any AdaptiveStrategy) async {
        adaptiveStrategies.append(strategy)
        eventContinuation.yield(.strategyAdded(strategy))
    }
    
    func removeAdaptiveStrategy(_ strategy: any AdaptiveStrategy) async {
    ...
    private func shouldPerformAdaptation(performance: PerformanceSnapshot) -> Bool {
        ...
        if performance.frameRateEfficiency < 0.8 ||
           ...
           performance.thermalState.rawValue >= ProcessInfo.ThermalState.serious.rawValue ||
    ...
    private func applyQualityLevelChange(_ level: QualityLevel) async {
        ...
        var newConfiguration = applyQualityLevel(level, to: baseConfig)
    ...
    private func getCurrentPerformanceSnapshot() async -> PerformanceSnapshot {
        ...
        // ThermalManager now returns ProcessInfo.ThermalState directly
        let thermalState = thermalManagerState
    ...
    private func setupQualityLevels() {
        qualityLevels = [
            ...
            QualityLevel(
                ...
                maxMemoryUsage: 0.8,
                maxThermalState: ProcessInfo.ThermalState.serious,
                minBatteryLevel: 0.3,
            ...
            QualityLevel(
                ...
                maxMemoryUsage: 0.9,
                maxThermalState: ProcessInfo.ThermalState.critical,
                minBatteryLevel: 0.1,
...
enum AdaptiveQualityEvent: Sendable {
    ...
    case strategyAdded(any AdaptiveStrategy)
    case strategyRemoved(any AdaptiveStrategy)
...
protocol AdaptiveStrategy: Sendable, Identifiable where ID == String {
...
struct ThermalAdaptationStrategy: AdaptiveStrategy {
    ...
    func apply(to configuration: inout CameraConfiguration, performance: PerformanceSnapshot?) async {
        ...
        if performance.thermalState == ProcessInfo.ThermalState.critical {
        ...
        } else if performance.thermalState == ProcessInfo.ThermalState.serious {
            configuration.hdrEnabled = false
...
struct MemoryAdaptationStrategy: AdaptiveStrategy {
    ...
    func apply(to configuration: inout CameraConfiguration, performance: PerformanceSnapshot?) async {
        ...
        if performance.memoryUsage > 0.9 {
            ...
            configuration.quality = VideoQuality.hd1080
            configuration.previewQuality = PreviewQuality.medium
        }
    }
}
