...
actor CameraCaptureService: Sendable {
    ...
    private var frameProcessors: [any FrameProcessor] = []
    private var frameFilters: [any FrameFilter] = []
    private var frameTransforms: [any FrameTransform] = []
    ...
    init(configuration: CameraConfiguration) {
        ...
        // Initialize processing components asynchronously
        Task {
            await setupProcessingComponents()
            await setupMetalProcessing()
        }
    }
    ...
    private func createFrameBufferPool() async throws {
        let width = Int(configuration.quality.dimensions.width)
        let height = Int(configuration.quality.dimensions.height)
...
enum CaptureEvent: Sendable {
    ...
    case processorAdded(any FrameProcessor)
    case processorRemoved(any FrameProcessor)
    case filterAdded(any FrameFilter)
    case filterRemoved(any FrameFilter)
    case transformAdded(any FrameTransform)
    case transformRemoved(any FrameTransform)
...
protocol FrameProcessor: Sendable, Identifiable where ID == String {
...
protocol FrameFilter: Sendable, Identifiable where ID == String {
...
protocol FrameTransform: Sendable, Identifiable where ID == String {
    func apply(to frame: DualCameraFrame, configuration: CameraConfiguration) async throws -> DualCameraFrame
}
