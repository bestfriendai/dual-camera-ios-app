...
class PerformanceMonitor: @unchecked Sendable {
    ...
    private var frameRateBuffer: [CFTimeInterval] = Array(repeating: 0.0, count: 60)
    ...
    // CPU monitoring
    private var cpuUsageBuffer: [Double] = Array(repeating: 0.0, count: 30)
    ...
    private var thermalStateHistory: [(Date, ProcessInfo.ThermalState)] = Array(repeating: (Date.distantPast, .nominal), count: 100)
    ...
    @MainActor private func setupBatteryMonitoring() {
    ...
    @MainActor @objc private func batteryStateChanged() {
        batteryLevel = UIDevice.current.batteryLevel
        batteryState = UIDevice.current.batteryState
