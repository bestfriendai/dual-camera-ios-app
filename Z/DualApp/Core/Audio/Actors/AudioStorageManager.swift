...
actor AudioStorageManager: Sendable {
    ...
    func loadRecording(at url: URL) async throws -> AudioRecordingInfo {
        ...
        let recording = AudioRecordingInfo(
            ...
            channels: UInt32(metadata.channels ?? 2),
    ...
    func deleteRecording(id: UUID) async throws {
        guard let recording = recordingsIndex[id.uuidString] else {
    ...
    func getWaveform(for recordingId: UUID) async -> AudioWaveform? {
        ...
        // Load from disk
        guard let recording = recordingsIndex[recordingId.uuidString] else {
            return nil
        }
    ...
    func exportRecording(_ recording: AudioRecordingInfo, to format: AudioFormat, quality: AudioQuality) async throws -> URL {
        ...
        // Export
        do {
            try await exportSession.export(to: exportURL, as: format.avFileType)
        } catch {
            throw AudioError.exportFailed(error.localizedDescription)
        }
    ...
    private func updateStorageMetrics() async {
        ...
        do {
            let contents = try fileManager.contentsOfDirectory(at: recordingsURL, includingPropertiesForKeys: [.fileSizeKey])
            usedSpace = 0
            totalRecordings = 0
    ...
    private func extractMetadata(from asset: AVAsset) async throws -> AudioMetadata {
        ...
        // Load tracks for format metadata
        ...
        if let audioTrack = tracks.first {
            ...
            if let formatDescription = formatDescriptions.first {
                ...
                if let asbd = audioStreamBasicDescription {
                    metadata.sampleRate = asbd.pointee.mSampleRate
    ...
    private func cleanupTemporaryFiles() async {
        ...
        guard let contents = try? fileManager.contentsOfDirectory(at: tempURL, includingPropertiesForKeys: [.contentModificationDateKey]) else {
            return
        }
    ...
    private func cleanupUnusedWaveforms() async {
        ...
        guard let contents = try? fileManager.contentsOfDirectory(at: waveformsURL, includingPropertiesForKeys: nil) else {
            return
        }
...
extension AudioFormat {
    var avFileType: AVFileType {
        ...
        case .alac:
            return .m4a
        case .flac:
            return .wav  // FLAC not available, use WAV as alternative
        }
    }
}
