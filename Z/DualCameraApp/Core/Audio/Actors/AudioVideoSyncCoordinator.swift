...
@MainActor
actor AudioVideoSyncCoordinator: Sendable {
    ...
    private func getSynchronizedAudioVideoBuffers() async -> (AudioFrame?, VideoFrame?) {
        ...
        if abs(timeDiff) <= tolerance {
            ...
            await audioBufferQueue.remove()
            await videoBufferQueue.remove()
            return (audio, video)
    ...
    private func calculateAverageBufferLatency(for type: BufferType) async -> TimeInterval {
        ...
        case .audio:
            ...
            return CMClockGetTime(currentTime) - CMClockGetTime(audio.clockTime)
            
        case .video:
            let videoFrame = await videoBufferQueue.peek()
            guard let video = videoFrame else { return 0.0 }
            return CMClockGetTime(currentTime) - CMClockGetTime(video.clockTime)
        }
    }
    ...
    private func applyDriftCorrection() async {
        ...
        if currentDrift > 0 {
            ...
            let framesToDrop = Int(correctionAmount / (1.0 / 30.0)) // Assuming 30fps
    ...
    private func createPCMBuffer(from sampleBuffer: CMSampleBuffer) -> AVAudioPCMBuffer? {
        ...
        // Copy sample buffer data to PCM buffer
