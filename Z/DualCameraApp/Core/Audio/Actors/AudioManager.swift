...
actor AudioManager: Sendable {
    ...
    init() {
        ...
        self.audioFocusManager = AudioFocusManager()
...
// MARK: - Audio Focus Event

struct AudioFocusEvent: Sendable, Codable {
    let type: AudioFocusType
    let timestamp: Date
    
    init(type: AudioFocusType, timestamp: Date = Date()) {
        self.type = type
        self.timestamp = timestamp
    }
}

enum AudioFocusType: String, CaseIterable, Sendable, Codable {
    case gained = "gained"
    case lost = "lost"
...
actor AudioFocusManager: Sendable {
