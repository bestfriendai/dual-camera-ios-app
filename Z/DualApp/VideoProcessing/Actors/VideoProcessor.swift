...
actor VideoProcessor: Sendable {
    ...
    func composeVideos(_ videos: [URL], outputURL: URL, layout: VideoCompositionLayout) async throws -> URL {
        ...
        let job = VideoProcessingJob(
            id: UUID(),
            inputURL: videos.first ?? URL(fileURLWithPath: ""),
            outputURL: outputURL,
            configuration: config,
            status: .queued,
            progress: 0.0
        )
        job.inputURLs = videos
        ...
        while reader.status == .reading {
            autoreleasepool {
                ...
                if presentationTime >= nextFrameTime {
                    ...
                    if let cgImage = ciContext.createCGImage(CIImage(cvPixelBuffer: imageBuffer), from: CGRect(origin: .zero, size: videoTrack.naturalSize)) {
    ...
    func createPictureInPicture(_ mainVideoURL: URL, pipVideoURL: URL, outputURL: URL, pipPosition: PictureInPicturePosition) async throws -> URL {
        ...
        job.inputURLs = [mainVideoURL, pipVideoURL]
    ...
    func createSideBySide(_ leftVideoURL: URL, rightVideoURL: URL, outputURL: URL) async throws -> URL {
        ...
        job.inputURLs = [leftVideoURL, rightVideoURL]
    ...
    func createSplitScreen(_ topVideoURL: URL, bottomVideoURL: URL, outputURL: URL) async throws -> URL {
        ...
        job.inputURLs = [topVideoURL, bottomVideoURL]
    ...
    func createOverlay(_ baseVideoURL: URL, overlayVideoURL: URL, outputURL: URL, blendMode: VideoBlendMode) async throws -> URL {
        ...
        job.inputURLs = [baseVideoURL, overlayVideoURL]
    ...
    private func executeVideoCompositionJob(assets: [AVAsset], job: VideoProcessingJob) async throws -> URL {
        ...
        for (index, audioTrack) in audioTracks.enumerated() {
            ...
            guard let compositionAudioTrack = compositionAudioTrack,
                  let audioAsset = audioTrack.asset else {
                continue
            }
            try compositionAudioTrack.insertTimeRange(CMTimeRange(start: CMTime.zero, duration: audioAsset.duration), of: audioTrack, at: CMTime.zero)
        }
    ...
    private func createVideoComposition(videoTracks: [AVAssetTrack], layout: VideoCompositionLayout, frameRate: Int32) async throws -> (AVMutableCompositionTrack, AVMutableVideoComposition) {
        ...
        case .sideBySide:
            ...
            let instruction = AVMutableVideoCompositionInstruction()
            guard let videoAsset = videoTracks[0].asset,
            ...
            leftLayerInstruction.setTransform(leftTransform, at: CMTime.zero)
            ...
            rightLayerInstruction.setTransform(rightTransform, at: CMTime.zero)
        ...
        case .pictureInPicture(let position):
            ...
            let instruction = AVMutableVideoCompositionInstruction()
            guard let videoAsset = videoTracks[0].asset else {
                throw VideoProcessorError.compositionFailed
            }
            instruction.timeRange = CMTimeRange(start: CMTime.zero, duration: videoAsset.duration)
            ...
            mainLayerInstruction.setTransform(videoTracks[0].preferredTransform, at: CMTime.zero)
            ...
            pipLayerInstruction.setTransform(pipTransform, at: CMTime.zero)
        ...
        case .splitScreen:
            ...
            topLayerInstruction.setTransform(topTransform, at: CMTime.zero)
            ...
            bottomLayerInstruction.setTransform(bottomTransform, at: CMTime.zero)
        ...
        case .overlay(let blendMode):
            ...
            baseLayerInstruction.setTransform(videoTracks[0].preferredTransform, at: CMTime.zero)

            // Overlay video
            let overlayLayerInstruction = AVMutableVideoCompositionLayerInstruction(assetTrack: compositionVideoTrack)
            overlayLayerInstruction.setTransform(videoTracks[0].preferredTransform, at: CMTime.zero)
            overlayLayerInstruction.setOpacity(0.5, at: CMTime.zero) // Adjust opacity as needed
