...
class PermissionManager: @unchecked Sendable {
    ...
    func requestPhotoLibraryPermission(completion: @escaping @Sendable (Bool) -> Void) {
    ...
    func requestAllPermissions(completion: @escaping @Sendable (Bool, [PermissionType]) -> Void) {
        var deniedPermissions: [PermissionType] = []
        ...
        // Request camera permission if needed
        if currentCameraStatus == .notDetermined {
            let initialDenied = deniedPermissions
            requestCameraPermission { [weak self] cameraGranted in
                guard let self = self else { return }

                var updatedDeniedPermissions = initialDenied
                if !cameraGranted {
                    updatedDeniedPermissions.append(.camera)
                }

                // Continue with microphone
                self.requestMicrophoneIfNeeded(currentStatus: currentMicStatus, initialDenied: updatedDeniedPermissions, completion: completion)
            }
        } else if currentCameraStatus == .authorized {
            requestMicrophoneIfNeeded(currentStatus: currentMicStatus, initialDenied: deniedPermissions, completion: completion)
        } else {
            var updatedDeniedPermissions = deniedPermissions
            updatedDeniedPermissions.append(.camera)
            requestMicrophoneIfNeeded(currentStatus: currentMicStatus, initialDenied: updatedDeniedPermissions, completion: completion)
        }
    }
    
    private func requestMicrophoneIfNeeded(currentStatus: PermissionStatus, initialDenied: [PermissionType], completion: @escaping @Sendable (Bool, [PermissionType]) -> Void) {
    ...
    private func requestPhotoLibraryIfNeeded(initialDenied: [PermissionType], completion: @escaping @Sendable (Bool, [PermissionType]) -> Void) {
        let currentPhotoStatus = photoLibraryPermissionStatus()
    ...
    @MainActor func showPermissionAlert(for type: PermissionType, from viewController: UIViewController) {
    ...
    @MainActor func showMultiplePermissionsAlert(deniedPermissions: [PermissionType], from viewController: UIViewController) {
