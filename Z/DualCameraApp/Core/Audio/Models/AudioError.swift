...
enum AudioError: LocalizedError, Sendable, Codable {
...
extension AudioError {
    init?(from avError: NSError) {
        ...
        if domain == AVAudioSessionErrorDomain {
            ...
            case AVAudioSession.ErrorCode.mediaServicesWereLost.rawValue:
                self = .hardwareUnavailable
            case AVAudioSession.ErrorCode.mediaServicesWereReset.rawValue:
            ...
            case kAudioFileUnsupportedPropertyError:
                self = .formatInvalid
            case kAudioFileBadFilePathError:
                self = .fileNotFound(avError.localizedDescription)
            case kAudioFilePermissionError:
